...
# --- ระบบดาวน์โหลดโมเดลอัตโนมัติ ---
MODEL_PATH = os.path.join(BASE_DIR, 'unet_best_bleeding_weights.h5')
# ลิงก์ดาวน์โหลดตรงจาก Google Drive ที่เราทำกันไว้
...
def download_model():
    if not os.path.exists(MODEL_PATH):
        ...
        try:
            ...
            response.raise_for_status() # เช็คว่าลิงก์ยังใช้ได้ไหม
...
# รันฟังก์ชันดาวน์โหลดก่อนโหลดโมเดลเข้าเครื่อง
download_model()
...
def preprocess_image(image_path):
    ...
    if img is None:
        raise ValueError(f"ไม่สามารถอ่านรูปภาพได้ที่: {image_path}")
    img = cv2.resize(img, (256, 256)) 
    img = img.astype('float32') / 255.0
    img = np.expand_dims(img, axis=0)  # เพิ่มมิติ Batch
    img = np.expand_dims(img, axis=-1) # เพิ่มมิติ Channel
    return img
...
if __name__ == '__main__':
    # สำหรับรันบน Render หรือ Production จะใช้ Port จาก Environment
